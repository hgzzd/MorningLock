...
---
时间: 2026-02-26 13:58
迭代的主题: 通过 TDD 修复时段结束提前解锁与闹钟调度稳定性问题
迭代的细节: 先新增失败测试覆盖“时段结束但锁定进行中不应停服务”与“Android 12+ 无精确闹钟能力时降级调度”场景；随后在 AlarmReceiver 引入基于 LockState 的停服判定，避免结束闹钟提前移除覆盖层；在 AlarmScheduler 增加 shouldUseExactAlarm 策略与 SecurityException 回退到 setAndAllowWhileIdle；在 LockService.onDestroy 增加覆盖层存在时的 lockStartTimestamp 清理，防止状态残留；执行 ./gradlew test 与 ./gradlew :app:lintDebug，均通过。
___
